import { RaffleData } from "@/app/types";

export const raffleData: RaffleData = {
  id: 151,
  slug: "mt09n",
  title: "MT 09 YAMT 2026",
  description: "Lo nuevo de Yamaha",
  price: "1600",
  currency: "COP",
  progress: 77.31,
  tickets_number: 100000,
  whatsapp_link: "3174885708",
  instagram_link: "https://www.instagram.com/nestorcamachortx/",
  bannerImage: "https://wizzpro.s3.amazonaws.com/0.9857143326025831000208504.jpg",
  awards: [
    {
      id: 1294,
      title: "MT 09 TICKETS",
      description: "1 melon",
      image: "https://wizzpro.s3.amazonaws.com/0.69041249227655691000086369.jpg",
    },
    {
      id: 1295,
      title: "MT 09 TICKETS",
      description: "1 melon",
      image: "https://wizzpro.s3.amazonaws.com/0.88812588131235651000086370.jpg",
    },
    {
      id: 1297,
      title: "MT 09 TICKETS",
      description: "1 melon",
      image: "https://wizzpro.s3.amazonaws.com/0.0108693045756174111000086372.jpg",
    },
    {
      id: 1299,
      title: "MT 09 TICKETS",
      description: "1 melon",
      image: "https://wizzpro.s3.amazonaws.com/0.58755861407372991000086374.jpg",
    },
  ],
};
